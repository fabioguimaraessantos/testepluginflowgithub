create or replace function ufc_pms_integracao_fatura(p_cd_fatura in number,
                                                     p_login     in varchar2)
  ...
  v_fatu_tx_descricao   varchar2(100);
  ...
  -- Codigo Fonte Receita
  con_cod_fonte_receita_reemb   NUMBER := 2;
  ...
  cursor crs_itens_fatura is
    select itfa.itfa_cd_item_fatura,
           itfa.itfa_vl_item,
           itfa.tise_cd_tipo_servico,
           tise.tise_nm_tipo_servico,
           itfa.fore_cd_fonte_receita,
           df.empr_cd_empresa,
           itfa.itfa_tx_observacao,
           tise.TISE_TX_ERP,
           fr.FORE_TX_ERP
...
begin
  ...
  begin
    ...
    select fatu.fatu_in_status,
           fatu.defi_cd_deal_fiscal,
           fatu.fatu_tx_observacao
    ...
    select moed.moed_cd_erp_ind_fin,
           defi.clie_cd_cliente,
           defi.empr_cd_empresa
      into v_erp_ind_in_codigo, v_clie_cd_cliente, v_empr_cd_empresa
      from fatura fatu, deal_fiscal defi, moeda moed
     where fatu.fatu_cd_fatura = p_cd_fatura
       and fatu.defi_cd_deal_fiscal = defi.defi_cd_deal_fiscal
       and defi.moed_cd_moeda = moed.moed_cd_moeda(+);
    ...
    if (v_fatu_in_status = 'AP' or v_fatu_in_status = 'ER') then
      ...
      if (v_nr_cgc is not null) then
        ...
        select empr.empr_cd_erp_ccusto,
               empr.empr_cd_erp_cus_ide,
               empr.empr_cd_erp_projeto,
               empr.empr_cd_erp_proj_ide,
               empr.empr_cd_erp_filial,
               empr.EMPR_CD_ERP_TPD_CODIGO,
               empr.EMPR_CD_ERP_ACAO_CODIGO
          into v_erp_cd_ccusto,
               v_erp_cd_cus_ide,
               v_erp_cd_projeto,
               v_erp_cd_proj_ide,
               v_erp_cd_filial,
        ...
        if (v_erp_cd_ccusto is not null and v_erp_cd_cus_ide is not null and
           v_erp_cd_projeto is not null and v_erp_cd_proj_ide is not null and
           v_erp_cd_filial is not null and v_tpd_in_codigo is not null and
          ...
          -- insere PEDIDO
          insert into mgweb.VEN_PEDIDOVENDA@ln_mgweb
            (PED_IN_SEQUENCIA,
             CLI_ST_TIPOCODIGO,
             FIL_IN_CODIGO,
             PED_IN_CODIGO,
          ...
          insert into mgweb.ven_observacaopedido@ln_mgweb
            (pob_ch_tipoobservacao, ped_in_sequencia, pob_st_observacao)
          values
            (con_pob_ch_tipoobservacao,
             v_ped_in_sequencia,
             v_fatu_tx_observacao);
          ...
          for reg in crs_itens_fatura loop
            ...
            -- monta a descricao do item de pedido
            if (reg.fore_cd_fonte_receita = con_cod_fonte_receita_reemb) then
              v_fatu_tx_descricao := CONCAT(CONCAT(reg.TISE_TX_ERP, ' - '), reg.FORE_TX_ERP);
            else
              v_fatu_tx_descricao := reg.TISE_TX_ERP;
            end if;
          
            insert into mgweb.VEN_ITEMPEDIDOVENDA@ln_mgweb
              (PED_IN_SEQUENCIA,
               ITP_IN_SEQUENCIA,
               APL_IN_CODIGO,
               PROJ_IDE_ST_CODIGO,
            ...
            -- Insere a Obsrevação no Item do Pedido
            insert into mgweb.VEN_OBSITEMPEDIDO@ln_mgweb
              (ped_in_sequencia,
               itp_in_sequencia,
               oip_ch_tipoobservacao,
               ito_st_observacao)
            values
              (v_ped_in_sequencia,
               v_itp_in_sequencia,
               'N',
               reg.itfa_tx_observacao);
          ...
          if (crs_itens_fatura_ot%notfound = false) then
            ...
            select nvl(max(PED_IN_SEQUENCIA), 0) + 1
              into v_ped_in_sequencia
              from mgweb.VEN_PEDIDOVENDA@ln_mgweb;
            v_error_num := 0;
            ...
            loop
              ...
              select infp_cd_erp_item, infp_cd_erp_servico
                into v_infp_cd_erp_item, v_infp_cd_erp_servico
                from integ_fatura_parametro i
               where i.tise_cd_tipo_servico =
                     v_it_fatura.tise_cd_tipo_servico
                 and i.fore_cd_fonte_receita =
                     v_it_fatura.fore_cd_fonte_receita
                 and i.empr_cd_empresa = v_it_fatura.empr_cd_empresa;

              v_fatu_tx_descricao := CONCAT(CONCAT(v_it_fatura.TISE_TX_ERP, ' - '), v_it_fatura.FORE_TX_ERP);

              -- Busca o centro de custo e projeto da fatura, pois neste caso não é usado o padrão.
              SELECT cc.CUS_IN_REDUZIDO, cc.CUS_IDE_ST_CODIGO, pp.COD_IN_PROJETO, pp.PRO_IDE_ST_CODIGO 
              INTO v_erp_cd_ccusto, v_erp_cd_cus_ide, v_erp_cd_projeto, v_erp_cd_proj_ide
              FROM ITEM_FATURA it
              INNER JOIN CONTRATO_PRATICA cp ON it.COPR_CD_CONTRATO_PRATICA = cp.COPR_CD_CONTRATO_PRATICA
              INNER JOIN CONVERGENCIA c ON cp.COPR_CD_CONTRATO_PRATICA = c.COPR_CD_CONTRATO_PRATICA
              INNER JOIN VW_MEGA_CCUSTO cc ON c.CONV_CD_CCUSTO_MEGA = cc.CUS_IN_REDUZIDO
              INNER JOIN VW_MEGA_PROJETO pp ON c.CONV_CD_PROJETO_MEGA = pp.COD_IN_PROJETO
              WHERE it.ITFA_CD_ITEM_FATURA = v_it_fatura.ITFA_CD_ITEM_FATURA;
              ...
              -- Insere a Obsrevação no Item do Pedido
              insert into VEN_OBSITEMPEDIDO@ln_mgweb.cit
                (ped_in_sequencia,
                 itp_in_sequencia,
                 oip_ch_tipoobservacao,
                 ito_st_observacao)
              values
                (v_ped_in_sequencia,
                 v_itp_in_sequencia,
                 'N',
                 v_it_fatura.itfa_tx_observacao);
          ...
          -- v_erp_cd_ccusto, outros...
          v_message := 'Empresa Id: ' || v_empr_cd_empresa ||
                       ' possui parâmetros inválidos';
          update fatura fatu
        ...
        -- v_nr_cgc
        v_message := 'Cliente id: ' || v_clie_cd_cliente ||
                     ' com cnpj nulo';
        update fatura fatu
           set fatu.fatu_in_status = 'ER', fatu.fatu_tx_error = v_message
         where fatu.fatu_cd_fatura = p_cd_fatura;
  ...
  if (p_result = 1) then
    insert into historico_fatura
      (hifa_cd_historico_fatura,
       fatu_cd_fatura,
       hifa_cd_autor,
       hifa_dt_alteracao,
       hifa_in_status_anterior,
       hifa_in_status_atual)
    values
      (sq_hifa_cd_historico_fatura.nextval,
       p_cd_fatura,
       p_login,
       sysdate,
       v_fatu_in_status,
