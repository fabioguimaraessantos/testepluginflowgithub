create or replace view vw_item_hrs_fat_deal as
select
p.pess_cd_pessoa,
ap.alpe_pr_alocacao_periodo,
pv.peve_cd_perfil_vendido,
case when ( select count(*) peve_vl_perfil_vendido
from item_tabela_preco itp
where itp.tapr_cd_tabela_preco = tp.tapr_cd_tabela_preco and
itp.peve_cd_perfil_vendido = pv.peve_cd_perfil_vendido ) = 0 then 0
else ( select itp.ittp_vl_item_tb_preco
from item_tabela_preco itp
where itp.tapr_cd_tabela_preco = tp.tapr_cd_tabela_preco and
itp.peve_cd_perfil_vendido = pv.peve_cd_perfil_vendido ) end peve_vl_perfil_vendido,
...
and tp.copr_cd_contrato_pratica = ma.copr_cd_contrato_pratica
