create or replace view vw_consolida_receita_pms_usd as
  ...
  from  (select origem,
       ...
       from ( select 'Q1' origem,
               ...
               from ( select --------------------------------------------------------------
                             ...
                             fat.valor_bonus_brl,
                             --
                             row_number() over( partition by inf_rec.cd_contrato_pratica, inf_rec.dt_mes, inf_rec.aloc_in_estagio, inf_rec.resource_code
                                                order by inf_rec.cd_contrato_pratica, inf_rec.dt_mes, inf_rec.aloc_in_estagio, inf_rec.resource_code ) nlinha
                             ...
                             from ( select inf1.origem,
                                           ...
                                           case when inf1.source = 'REVENUE' and origem = 'Q11'
                                                ...
                                                -- fechado e n?o tem receita, n?o mostrando mais do ALLOCATION MAP
                                                               ...
                                                               then sum( vlr_preco_al_map ) over( partition by inf1.cd_contrato_pratica, inf1.dt_mes, inf1.aloc_in_estagio, inf1.resource_code )
                                         ...
                                         /*  inf1.cd_grupo_custo,
                                           inf1.nm_grupo_custo,*/
                                           row_number() over( partition by inf1.cd_contrato_pratica, inf1.dt_mes, inf1.aloc_in_estagio, inf1.resource_code
                                                              order by inf1.cd_contrato_pratica, inf1.dt_mes, inf1.aloc_in_estagio desc, inf1.resource_code ) nlinha
                                    from ( select origem,
                                           ...
                                           from ( select 'Q11'                              origem,
                                                  ...
                                                  from ( select rec.cd_contrato_pratica,
                                                                ...
                                                                rec.dt_mes,
                                                                rec.vlr_preco,
                                                                sum( rec.valor_fte ) over( partition by rec.cd_contrato_pratica, rec.dt_mes, rec.resource_code )  valor_fte,
                                                                0 valor_ut_rate,
                                                       ...
                                                       ( select pl.cd_contrato_pratica,
                                                                ...
                                                                sum( pl.valor_fte_ut_rate ) over( partition by pl.cd_contrato_pratica, pl.dt_mes, pl.aloc_in_estagio, pl.resource_code ) valor_fte_ut_rate,
                                                                pl.aloc_in_estagio,
                                                         ...
                                                         /*where --pl.nome_contrato_pratica = 'Coca Cola-ADS' and
                                                               pl.dt_mes = '01-jan-2010'*/ ) pl
                                                  where pl.cd_contrato_pratica = rec.cd_contrato_pratica (+) and
                                                        pl.dt_mes              = rec.dt_mes (+) and
                                                        pl.resource_code       = rec.resource_code (+) )
                                           union all
                                           select origem,
                                                  case when cd_cont_pratica_rev is not null
                                                            then cd_cont_pratica_rev
...
--       'AJ_REC' aloc_in_estagio,  Trocado a pedido do Boica
       inf.resource_code,
       inf.resource_name,
       ...
       0 valor_fte_ut_rate,
       'Actual' source,
--     'AJ_REC' source,  Trocado a pedido do Boica
                        ...
                        from   (select cl.celu_cd_centro_lucro,
                               ...
                               (select inf.origem,
                                ...
                                from   (select cl.celu_cd_centro_lucro,
                                       ...
                                       (select 'AJ_REC' origem,
                                        ...
                                        from   moeda               mo,
                                               ...
                                               receita             r,
                                               ajuste_receita      ar
                                        where  ar.redf_cd_receita_dfiscal = rfd.redf_cd_receita_dfiscal and
                                               rfd.rece_cd_receita = r.rece_cd_receita and
                                               cp.copr_cd_contrato_pratica = r.copr_cd_contrato_pratica and
                                               pr.prat_cd_pratica = cp.prat_cd_pratica and
                                               pe.pess_cd_login = ar.ajre_cd_login_autor and
                                               mo.moed_cd_moeda = cp.moed_cd_moeda_padrao
                                               ) inf/*,
                                            pessoa_grupo_custo  pgc,
                                            grupo_custo         gc*/
                                where  inf.dt_mes_ajuste between
                                       ...
                                       /*inf.pess_cd_pessoa = pgc.pess_cd_pessoa (+) and
                                       pgc.grcu_cd_grupo_custo = gc.grcu_cd_grupo_custo (+) and
                                       inf.dt_mes between pgc.pegc_dt_inicio(+) and (case when pgc.pegc_dt_fim(+) is not null then pgc.pegc_dt_fim(+)
                                                                                      else (select para_dt_parametro from   parametro where  para_cd_parametro = 1) end ) and*/
...
where  inf.cd_cotacao = cm.como_cd_cotacao_moeda(+)) recusd
