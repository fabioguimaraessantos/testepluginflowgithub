CREATE OR REPLACE PROCEDURE "USP_PMS_POSITION"  as

   log_data              date := trunc(sysdate);
   log_dt_carga_processo date;
   nome_carga            varchar2(50) := '  PMS_POSITION ';
   sqlerro               varchar2(200) := '';
   nome_proc             varchar2(200) := ' USP_PMS_POSITION ';
...
begin
   ...
   send_mail('andreiap@ciandt.com', assunto, mensagem);   
     ...
     begin
          execute immediate  'truncate table  TB_BI_PMS_POSITION';
     exception when others then
          erro := 1;

          --sqlerro  := sqlerrm;
          mensagem := mensagem || crlf || crlf ||
                              '[ERRO] ##### PMS.TB_BI_PMS_POSITION  - (LIMPAR A  TABELA)  ##### - ' || crlf ||
     ...
     if (erro = 0) then
               --- GRAVA NA TABELA
              begin
                    insert into  TB_BI_PMS_POSITION
                           select * from vw_bi_pms_position;
               ...
               exception when others then
                   ...
                   --sqlerro  := sqlerrm;
                   mensagem := mensagem || crlf || crlf ||
                                        '[ERRO] ##### PMS.TB_BI_PMS_POSITION   - (GRAVAR DADOS)  ##### - ' || crlf ||
                                        '       SQL Erro: ' || sqlerrm || crlf ||
                                        '       Data Execucao: ' ||
                                        to_char(sysdate, 'DD/MM/YYYY HH24:MI:SS');
               end;
