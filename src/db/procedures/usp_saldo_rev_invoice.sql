create or replace procedure usp_saldo_rev_invoice is
         ...
         (
                         ...
                         from   (select rdf.defi_cd_deal_fiscal cd_deal_fiscal,
                                 ...
                                 where  rdf.defi_cd_deal_fiscal = df.defi_cd_deal_fiscal
                                        ...
                                        and  rec.rece_in_versao  in ('PB', 'IN', 'PD')  --- AS RECEITAS EM WORKING NÃO PODEM ENTRAR NO PROCESSO
                                        and
                                        ...
                                        ( ---SÃO RECEITAS QUE NÃO TEM MAIS SALDO
                                         (select cd_receita
                                              from   (select cd_fiscal_deal,
                                                      ...
                                                      from   tb_bi_pms_saldo_rev_invoice s
                                                      where  s.valor_saldo_revenue <= 0)
                                              where  linha = 1))
                                 order  by df.defi_cd_deal_fiscal,
                                           rec.rece_dt_mes) rec,
                                vw_bi_pms_aux_fiscal_balance aux
...
begin ---usp_saldo_rev_invoice3;
   ...
   send_mail('lnoboru@ciandt.com','FISCAL BALANCE', 'Início do processo de Associação   '     || to_char(sysdate));     
   

   send_mail('alexandrel@ciandt.com','FISCAL BALANCE', 'Início do processo de Associação   '     || to_char(sysdate));     
   send_mail('tspadari@ciandt.com', 'FISCAL BALANCE', 'Início do processo de Associação   '     || to_char(sysdate));     
   send_mail('llino@ciandt.com', 'FISCAL BALANCE', 'Início do processo de Associação   '     || to_char(sysdate));     
   send_mail('andreiap@ciandt.com', 'FISCAL BALANCE', 'Início do processo de Associação   '     || to_char(sysdate));     
        ...
        --end if;
            ...
            if (saldo_revenue = 0) and (saldo_invoice > 0) then
                                        ...
                                        insert into fatura_receita
                                           ...
                                           (
                                               --var_receita. cd_receita,
                                              (select nvl(max(fare_cd_fatura_receita), 0) + 1 from  fatura_receita) ,
                                               var_cod_fatura,
                                               --var_receita.cd_deal_fiscal,
   ...
   send_mail('lnoboru@ciandt.com','FISCAL BALANCE', 'Fim do processo de Associação    '    ||  to_char(sysdate));      
   
   send_mail('alexandrel@ciandt.com','FISCAL BALANCE', 'Fim do processo de Associação   '     || to_char(sysdate));     
   send_mail('tspadari@ciandt.com', 'FISCAL BALANCE', 'Fim do processo de Associação   '     || to_char(sysdate));     
   send_mail('llino@ciandt.com', 'FISCAL BALANCE', 'Fim do processo de Associação   '     || to_char(sysdate));     
   send_mail('andreiap@ciandt.com', 'FISCAL BALANCE', 'Fim do processo de Associação   '     || to_char(sysdate));    

end usp_saldo_rev_invoice;