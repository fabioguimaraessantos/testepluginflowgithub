CREATE OR REPLACE PROCEDURE "USP_PMS_PROFITLOSS_CARGA3" (flag number) as
--   INTERVAL: trunc(sysdate+1) + 2.5/24
   -- Informac?es para LOG
   log_data              date := trunc(sysdate);
   log_dt_carga_processo date;
   nome_carga            varchar2(50) := '(7.3) PROFITLOSS';
